...
impl Protocol {
    pub fn new(name: &str, level: u8) -> Result<Protocol, Error> {
        match name {
            "MQIsdp" => match level {
...
impl QoS {
    ...
    #[inline]
    pub fn from_hd(hd: u8) -> Result<QoS, Error> {
...
impl PacketType {
    ...
    #[inline]
    pub fn from_hd(hd: u8) -> Result<PacketType, Error> {
        Self::from_u8(hd >> 4)
    }
}
...
#[cfg(test)]
mod test {
    use super::{PacketIdentifier, Protocol, QoS};
